...
public:
	...
	BEGIN_UPDATE_UI_MAP(CMainFrame)
        ...
        UPDATE_ELEMENT(ID_OPTION_EXPRESSMODE, UPDUI_MENUPOPUP)
	END_UPDATE_UI_MAP()

	BEGIN_MSG_MAP(CMainFrame)
		COMMAND_ID_HANDLER(ID_FILE_DUMPDATA, OnFileDumpdata)
		COMMAND_ID_HANDLER(ID_FILE_VIEWMOVE, OnViewMove)  //’Ç‰Á
		COMMAND_ID_HANDLER(ID_FILE_LOGDUMPCLEAR, OnLogDumpDataAndClear)  //’Ç‰Á
		COMMAND_ID_HANDLER(ID_FILE_LOGREAD, OnLogRead)  //’Ç‰Á
		COMMAND_ID_HANDLER(ID_FILE_OPENBACK, OnBackgroundRead)  //’Ç‰Á
		COMMAND_ID_HANDLER(ID_OPT_FORCESCAN, OnOptForcescan)
        COMMAND_ID_HANDLER(ID_OPTION_EXPRESSMODE, OnOptExpressMode)
		COMMAND_ID_HANDLER(ID_CMD_SCAN, OnCmdScan)
		COMMAND_ID_HANDLER(ID_CMD_VIEW, OnCmdView)  //’Ç‰Á
		...
		COMMAND_ID_HANDLER(ID_CMD_SET_LOGTERM, OnSetLogTerm)  //’Ç‰Á
	...
	LRESULT OnCmdView(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnOptForcescan(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
    LRESULT OnOptExpressMode(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnFileDumpdata(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnViewMove(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	BOOL OnMouseWheel(UINT nFlags, short zDelta, CPoint pt);
	LRESULT OnMouseWheel(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);

	LRESULT OnLogDumpDataAndClear(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnLogRead(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnSetLogTerm(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);
	LRESULT OnBackgroundRead(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled);;
